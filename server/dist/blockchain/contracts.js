...
export function getRegistry() {
    ...
    if (!config.registryAddress || !provider)
        return null;
...
export function getHook() {
    const config = getConfig();
    const provider = getProvider();
    if (!config.hookAddress || !provider)
        return null;
...
export function getInternalHook() {
    const config = getConfig();
    const provider = getProvider();
    if (!config.internalHookAddress || !provider)
        return null;
...
export function getRouter() {
    const config = getConfig();
    const provider = getProvider();
    if (!config.routerAddress || !provider)
        return null;
    return new ethers.Contract(config.routerAddress, ROUTER_ABI, provider);
}
export async function getAgentFromChain(jobId) {
    try {
...
export async function getInvestorPositionFromChain(jobId, investor) {
    try {
        const hook = getHook();
        if (!hook)
            return null;
...
export async function getInternalPoolStateFromChain(poolId) {
    try {
...
export async function getOptimalRouteFromChain(tokenIn, tokenOut, amountIn) {
    try {
