...
export function registerInvestorTools(server) {
    server.tool("fund_proposal", "Deposit capital to back a trading agent proposal", {
        ...
        if (!proposal) {
            return {
                content: [{ type: "text", text: JSON.stringify({ error: `Proposal ${proposal_id} not found` }) }],
                isError: true,
            };
        }
        if (proposal.status !== "funding") {
            return {
                content: [{ type: "text", text: JSON.stringify({ error: `Proposal ${proposal_id} is not accepting funding (status: ${proposal.status})` }) }],
                isError: true,
            };
        }
        ...
        if (amount > remaining) {
            return {
                content: [{ type: "text", text: JSON.stringify({ error: `Amount ${amount} exceeds remaining capacity ${remaining}` }) }],
                isError: true,
            };
        }
        return {
    ...
    server.tool("get_my_positions", "Get all active investment positions for the connected wallet", {
    ...
    server.tool("withdraw_position", "Withdraw from an investment position (may incur early exit penalties)", {
        proposal_id: z.string(),
        amount: z.number().positive(),
    ...
    server.tool("file_claim", "File a claim against a failed agent's collateral via ERC-8210", {
        proposal_id: z.string(),
        ...
        if (proposal.status !== "failed") {
            return {
                content: [{ type: "text", text: JSON.stringify({ error: `Claims can only be filed against failed proposals. Current status: ${proposal.status}` }) }],
                isError: true,
            };
        }
        ...
        if (claim_amount > maxClaim) {
            return {
                content: [{ type: "text", text: JSON.stringify({ error: `Claim amount ${claim_amount} exceeds available collateral ${maxClaim}` }) }],
                isError: true,
            };
        }
        return {
