import { z } from "zod";
import { getRegistry, getHook, getVault, statusName, tierName, } from "../blockchain/contracts.js";
function serializeAgent(agentId, raw) {
    return {
...
export function registerMarketplaceTools(server) {
    server.tool("list_agents", "List all trading agents registered on Lockstep. Iterates getAgent(1..nextAgentId-1) and skips slots that no longer resolve.", {
    ...
    server.tool("get_agent_details", "Get full details of a single agent, including committed collateral and current investor count.", {
        ...
        try {
            ...
            const [committedCollateral, investors, totalLiquidity] = await Promise.all([
                vault.committedCollateral(BigInt(agent_id)),
                hook.getJobInvestors(BigInt(agent_id)),
                hook.totalJobLiquidity(BigInt(agent_id)),
            ]);
            return {
                content: [{
                        type: "text",
