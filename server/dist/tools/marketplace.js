...
export function registerMarketplaceTools(server) {
    server.tool("list_proposals", "List active trading agent proposals with optional filters", {
        ...
        let results = getAllProposals();
        if (status)
            results = results.filter((p) => p.status === status);
        if (min_capital)
            results = results.filter((p) => p.capitalRequired >= min_capital);
        if (max_capital)
            results = results.filter((p) => p.capitalRequired <= max_capital);
        if (tier)
    ...
    server.tool("get_proposal_details", "Get full details of a specific trading agent proposal", {
    ...
    server.tool("evaluate_proposal", "Compute economic metrics for an investment in a proposal", {
        proposal_id: z.string(),
