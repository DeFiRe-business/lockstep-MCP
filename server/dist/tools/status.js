...
import { getRegistry, getEvalRegistry, getVault, statusName, } from "../blockchain/contracts.js";
export function registerStatusTools(server) {
    server.tool("get_protocol_stats", "Aggregate protocol-wide statistics. Iterates every agent on-chain. If BACKEND_URL is set, the backend's /api/stats endpoint is preferred for speed.", {}, async () => {
        try {
            const cfg = getConfig();
            if (cfg.backendUrl) {
            ...
            for (let id = 1; id < upper; id++) {
                try {
                    ...
                    try {
                        const c = await vault.committedCollateral(BigInt(id));
                        committed += c;
                    }
                    catch {
                        // ignore
            ...
            const totalSlashedFunds = await evalRegistry.totalSlashedFunds();
            const slashCount = await evalRegistry.slashCount();
            const minStake = await evalRegistry.minStake();
            return {
                content: [{
                        type: "text",
                        text: JSON.stringify({
