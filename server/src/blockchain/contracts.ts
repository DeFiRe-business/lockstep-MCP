...
export function getRouterAs(role: ActorRole): ethers.Contract {
  return getRouter().connect(getSigner(role)) as ethers.Contract;
}

export function getInternalHookAs(role: ActorRole): ethers.Contract {
  return getInternalHook().connect(getSigner(role)) as ethers.Contract;
}

// ─── Enum helpers (mirror ILockstepRegistry.Status / Tier) ────────────────────

export const STATUS_NAMES = ["Funding", "Active", "Completed", "Failed"] as const;
export const TIER_NAMES = ["Newcomer", "Verified", "Established"] as const;

export function statusName(s: number | bigint): string {
  const i = Number(s);
  return STATUS_NAMES[i] ?? `Unknown(${i})`;
}

export function tierName(t: number | bigint): string {
  const i = Number(t);
