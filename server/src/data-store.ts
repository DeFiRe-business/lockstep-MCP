...
const proposals: TradingAgentProposal[] = [
  {
    id: "prop-001",
    agentName: "AlphaArb",
...
const internalPools: InternalPool[] = [
  {
    pair: "ETH/USDC",
...
export function getAllProposals(): TradingAgentProposal[] {
  return proposals;
}

export function getProposalById(id: string): TradingAgentProposal | undefined {
  return proposals.find((p) => p.id === id);
}

export function getInternalPools(token?: string): InternalPool[] {
  if (!token) return internalPools;
  const t = token.toUpperCase();
  return internalPools.filter((p) => p.pair.includes(t));
}

export function getArbOpportunities(minProfitBps: number = 10, limit: number = 5): ArbOpportunity[] {
  const opps: ArbOpportunity[] = internalPools
    ...
    .map((p) => {
      ...
      return {
        ...
        direction: buyInternal
          ? ("buy_internal_sell_external" as const)
          : ("buy_external_sell_internal" as const),
...
export function getProtocolStatsData(): ProtocolStats {
  return {
    totalTvl: 3070000,
    activeAgents: 2,
...
export function getOpenPositions(walletAddress: string): OpenPosition[] {
  return [
    {
      positionId: "pos-001",
      proposalId: "prop-001",
...
export function computeRouteResult(tokenIn: string, tokenOut: string, amountIn: number): {
