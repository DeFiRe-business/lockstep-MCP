...
export function registerAdminTools(server: McpServer): void {
  server.tool(
    "register_evaluator",
    "Register (or top up) an evaluator on LockstepEvaluatorRegistry. onlyOwner — requires ADMIN_PRIVATE_KEY (or MCP_PRIVATE_KEY).",
    {
  ...
  server.tool(
    ...
    "Withdraw stake from an evaluator. onlyOwner. Requires ADMIN_PRIVATE_KEY (or MCP_PRIVATE_KEY).",
    {
      evaluator: z.string(),
  ...
  server.tool(
    "slash_evaluator",
    "Slash an evaluator's stake for a specific jobId. Records a SlashRecord and emits EvaluatorSlashed. onlyOwner. Requires ADMIN_PRIVATE_KEY (or MCP_PRIVATE_KEY).",
    {
      evaluator: z.string(),
  ...
  server.tool(
    "set_min_stake",
    "Update the LockstepEvaluatorRegistry minimum stake. onlyOwner. Requires ADMIN_PRIVATE_KEY (or MCP_PRIVATE_KEY).",
    {
  ...
  server.tool(
    "evaluate_job",
    "Force-trigger PerformanceEvaluator.evaluate(jobId). Normally called by a keeper bot at the commitment deadline; exposed via the admin role for manual testing. Requires ADMIN_PRIVATE_KEY (or MCP_PRIVATE_KEY).",
    {
      job_id: z.number().int().positive(),
    },
    async ({ job_id }) => {
