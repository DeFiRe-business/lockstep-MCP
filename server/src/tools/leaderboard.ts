...
interface LeaderboardEntry {
  ...
  last_final_balance: string | null;
...
export function registerLeaderboardTools(server: McpServer): void {
  server.tool(
    "get_leaderboard",
    "Ranked agent leaderboard. Default behaviour iterates getAgent + Evaluated events on-chain. If BACKEND_URL is set, the backend's /api/leaderboard endpoint is preferred for speed.",
    {
    ...
    async ({ sort_by, limit }) => {
      try {
        ...
        const byJob = new Map<string, { success: boolean; finalBalance: bigint; minExpected: bigint }[]>();
        for (const ev of evaluatedEvents) {
          if (!("args" in ev) || !ev.args) continue;
          const jobId = (ev.args.jobId as bigint).toString();
          const list = byJob.get(jobId) ?? [];
          list.push({
            success: ev.args.success as boolean,
            finalBalance: ev.args.finalBalance as bigint,
            minExpected: ev.args.minExpected as bigint,
          });
          ...
          try {
            ...
            if (lastEval) {
              const initial = lastEval.minExpected > 0n
                ? (lastEval.minExpected * 10000n) / (10000n + (raw.minReturnBps as bigint))
                : 0n;
              if (initial > 0n) {
